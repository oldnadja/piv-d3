...
define("app/module", [ "angular", "config-angular", "angular-ui-router" ], function(angular, configAngular) {
    ...
    this.d3 = Zo;
}(), define("piv/controllers/GraphController", [ "underscore", "d3" ], function(_) {
    "use strict";
    ...
    return prototype.initialize = function() {
        ...
        vectorsPromise.$promise.then(function(result) {
            self.vectors = result.vectors.points, self.links = result.vectors.links, self.drawLinks();
        });
    }, prototype.getLinks = function() {
        for (var self = this, links = [], i = 0; i < self.links.length; i++) links[i] = {}, 
        links[i].source = _.findWhere(self.vectors, {
            id: self.links[i].source
        }), links[i].target = _.findWhere(self.vectors, {
            id: self.links[i].target
        });
        return links;
        ...
        function createDefs(defs) {
            ...
            feMerge.append("svg:feMergeNode"), feMerge.append("svg:feMergeNode").attr("in", "SourceGraphic");
        }
        function drawCurve(d) {
            console.log(d);
            var slope = Math.atan2(+d3.select("#circle" + d.target).attr("cy") - d3.select("#circle" + d.source).attr("cy"), +d3.select("#circle" + d.target).attr("cx") - d3.select("#circle" + d.source).attr("cx")), slopePlus90 = Math.atan2(+d3.select("#circle" + d.target).attr("cy") - d3.select("#circle" + d.source).attr("cy"), +d3.select("#circle" + d.target).attr("cx") - d3.select("#circle" + d.source).attr("cx")) + Math.PI / 2, sourceX = +d3.select("#circle" + d.source).attr("cx"), sourceY = +d3.select("#circle" + d.source).attr("cy"), targetX = +d3.select("#circle" + d.target).attr("cx"), targetY = +d3.select("#circle" + d.target).attr("cy"), arrowOffset = 15, points = [];
            return points.push([ sourceX + radius * Math.cos(slope) - strength_scale(d.strength) * Math.cos(slopePlus90), sourceY + radius * Math.sin(slope) - strength_scale(d.strength) * Math.sin(slopePlus90) ]), 
            ...
            d3LineLinear(points) + "Z";
        }
        var self = this;
        console.log(this.vectors);
        var width = 700, height = 400, radius = 2, numCircles = this.vectors.length, d3LineLinear = d3.svg.line().interpolate("linear"), links = self.getLinks();
        console.log(links);
        var drag = d3.behavior.drag().origin(Object).on("drag", function() {}), circles = d3.range(numCircles).map(function(i) {
            return self.vectors[i] ? [ self.vectors[i].coords[0], self.vectors[i].coords[1] ] : void 0;
        }), strength_scale = d3.scale.linear().range([ 2, 10 ]).domain([ 0, d3.max(links, function(d) {
            return 10 * d.strength;
        }) ]), svg = d3.select(".field.gray-border").append("svg").attr("width", width).attr("height", height), g_lines = svg.append("g").attr("class", "lines"), g_circles = svg.append("g").attr("class", "circles");
        createDefs(svg.append("svg:defs")), $.each(circles, function(i, d) {
        ...
        }), g_lines.selectAll(".link_line").data(links).enter().append("path").attr("d", function(d) {
            return drawCurve(d);
        });
